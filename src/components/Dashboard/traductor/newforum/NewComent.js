...
const NewComment = ({ comment, user, handleReply }) => {
    ...
    const deleteComment = () => {
        if (confirm("Confirm You Want To Delete Comment.")) {
            Axios.delete(
                `http://127.0.0.1:8000/newforum/${comment.post}/comments/${comment.id}`,
                {
                    headers: {
    ...
    const submitComment = (e) => {
        ...
        Axios.post(
            `http://127.0.0.1:8000/newforum/${comment.post}/comments/`,
            {
                createdBy: user.id,
                parent: comment.id,
                post: comment.post,
