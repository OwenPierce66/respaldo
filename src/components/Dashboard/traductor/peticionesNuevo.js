...
const Peticiones = () => {
  ...
  const { data: feedData, isLoading, isError } = useGetFeedQuery();
  const tasksArray = feedData?.tasks ? Object.values(feedData.tasks) : [];
...
const closeAddModal = () => setIsAddModalOpen(false);
  // ✅ useEffect siempre en top-level
  useEffect(() => {
  ...
  if (isError) return <div>Error cargando el feed</div>;

  // Sacamos las tasks normalizadas y las convertimos en array
...
// }
  ...
  try {
    ...
    const tasksFromFeed = response.data; // ya viene normalizado desde el backend

    // Filtro directo (ya no necesito normalizeTasks)
    const filtered = tasksFromFeed.filter((item) => {
      const taskCategories = item.categories ? item.categories.split(',') : [];

      return (
        (selectedCategory === "Todas las categorías" || taskCategories.includes(selectedCategory)) &&
        (peticionajena === "" || item.username === peticionajena)
      );
    });

    setNormalizedTasks(filtered); // antes hacías merge, ahora el BE ya manda el feed completo
  } catch (error) {
    console.error("Error fetching tasks:", error);
  }
};
...
//     const responseSharedTasksUnidicado = await axios.get(`http://127.0.0.1:8000/api/feed/`, {
//       headers: { Authorization: `Token ${localStorage.getItem("userTokenLG")}` },
//     });

//     const newTasks = responseTasks.data;
//     const newSharedTasks = responseSharedTasks.data;
//     const newSharedTasksUnificado = responseSharedTasks.data;

//     console.log("las unificadas del backend", newSharedTasksUnificado);
...
// };
  ...
  const taskCategories = task.categories
    ? task.categories.split(",").map((cat) => cat.trim().toLowerCase())
    : [];

  // búsqueda
  const username = task.username ? task.username.toLowerCase() : "";
  const title = task.title ? task.title.toLowerCase() : "";
  const searchLower = combinedSearchTerm.trim().toLowerCase();

  const categoryMatch =
    selectedCategory === "Todas las categorías" ||
    taskCategories.includes(selectedCategory.toLowerCase());

  const searchMatch =
    username.includes(searchLower) || title.includes(searchLower);

  const peticionAjenaMatch =
          ...
          <div className="redondear" style={{ padding: "6px" }}>
            ...
            {link.video && (
              <video controls className="testimonial-video">
                <source src={getMediaUrl(link.video)} type="video/mp4" />
                Your browser does not support the video tag.
              </video>
            )}
            {link.image && (
              <img src={getMediaUrl(link.image)} alt="Imagen" onClick={() => imageSelect(link.image)} className="imagenPeticion" style={{ height: "100px", width: "100px" }} />
            )}
