...
import {
  faPlay, faPen, faUsers, faUser, faEnvelope, faHeart, faTrash, faImage,
  faGlobe, faArrowRight, faBars, faMinus, faPlus, faSearch, faUserCheck,
...
import { API_BASE, toSrc, getFirstImage, getFirstVideo } from "./utils/media";
import {
  APP_USER_ID,
  APP_USERNAME,
  getTierKey,
...
const isAdmin = useMemo(() => {
  // usuario del endpoint /get-user/
  const u = usuario?.user ?? usuario ?? null;

  // user de redux / props
  const ru = user?.user ?? user ?? null;

  return (
    toBool(u?.is_superuser) ||
    toBool(u?.is_staff) ||
    toBool(u?.is_admin) ||       // por si agregas bandera directa
    toBool(ru?.is_superuser) ||
    toBool(ru?.is_staff) ||
    toBool(ru?.is_admin)
  );
}, [usuario, user]);
...
const [shareModalFilter, setShareModalFilter] = useState('all');
...
const adminAppLikeTask = async (taskId, liked) => {
  try {
    const token = localStorage.getItem("userTokenLG");
    const { data } = await axios.post(
      `${API_BASE}/api/admin/tasks/${taskId}/app-like/`,
      { liked },
    ...
    setItems(prev =>
      prev.map(t => {
        ...
        const nextLikeSet = Array.isArray(t.like_set) ? [...t.like_set] : [];
        const exists = nextLikeSet.some(x => x?.user?.id === APP_USER_ID);

        if (liked && !exists) {
          nextLikeSet.push({ user: { id: APP_USER_ID, username: APP_USERNAME } });
        }

        if (!liked && exists) {
          const filtered = nextLikeSet.filter(x => x?.user?.id !== APP_USER_ID);
          return { ...t, likes_count: data.likes_count, like_set: filtered };
        }

        return { ...t, likes_count: data.likes_count, like_set: nextLikeSet };
      })
    );
  } catch (err) {
    console.error("❌ adminAppLikeTask error:", err?.response?.data || err);
  }
};



// ✅ REEMPLAZA tu likesModalCounts por este
const likesModalCounts = useMemo(() => {
  // arma todos los keys que existen en tierTabs (incluye "all")
...
// ✅ REEMPLAZA tu listUsersFiltered por este
...
// ✅ REEMPLAZA tu shareModalCounts por este
const shareModalCounts = useMemo(() => {
  const counts = (tierTabs || []).reduce((acc, t) => {
    acc[t.key] = 0;
    return acc;
  }, {});
...
// ✅ REEMPLAZA tu sharedUsersFiltered por este
const sharedUsersFiltered = useMemo(() => {
  const arr = Array.isArray(sharedUsers) ? sharedUsers : [];
  if (shareModalFilter === "all") return arr;
  return arr.filter((u) => getTierKey(u) === shareModalFilter);
}, [sharedUsers, shareModalFilter]);
...
const setUserVerified = async (targetUserId, makeVerified = true) => {
  ...
  try {
    ...
    // ✅ Endpoint admin
    const url = `http://127.0.0.1:8000/api/admin/users/${targetUserId}/verify/`;
    ...
    // refresca el feed (mantiene UI consistente)
    refetch();

    console.log(`✅ user ${targetUserId} verified=${makeVerified}`);
  } catch (err) {
    console.error("❌ Error al verificar usuario:", err?.response?.data || err);
  }
};

const setUserRecommended = async (targetUserId, makeRecommended = true) => {
  if (!targetUserId) return;

  try {
    const token = localStorage.getItem("userTokenLG");

    const url = `http://127.0.0.1:8000/api/admin/users/${targetUserId}/recommend/`;
    ...
    refetch();
    console.log(`✅ user ${targetUserId} recommended=${makeRecommended}`);
  } catch (err) {
    console.error("❌ Error al recomendar usuario:", err?.response?.data || err);
  }
};
...
useEffect(() => {
  console.log("[ADMIN DEBUG] usuario:", usuario);
  console.log("[ADMIN DEBUG] redux user:", user);
  console.log("[ADMIN DEBUG] isAdmin:", isAdmin);
...
console.log("GET-USER user payload:", response.data?.user);
...
console.log("NORM first:", normalized?.[0]);
console.log("user_image raw:", normalized?.[0]?._orig?.user_image);
console.log("avatar computed:", getUserAvatarSrc(normalized?.[0]));
  } catch (err) {
...
<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64">
  ...
  <rect x="14" y="40" width="36" height="18" rx="9" fill="#adb5bd"/>
</svg>
`.trim());
        ...
        <div className={"importante-separedor"} style={{ paddingTop: "0px" }}>
          ...
          <div className="botonesAbajo">
            ...
            <div>
              <TaskFilterMenu
                ...
                setMostrarSoloFavoritosUsuarioSeleccionado={setMostrarSoloFavoritosUsuarioSeleccionado}
          ...
          {peticionajena && (
            <button onClick={() => {
              ...
              setPortadasUsuarioSeleccionado([]);
              setImagenFijaUsuarioSeleccionado(null);
            }}>
            ...
            {mostrarUsuarios ? (
              ...
              filteredTasks
                ...
                .map(link => {
                  ...
                  const appHasLiked = (link?.like_set || []).some(l => (l?.user?.id === APP_USER_ID));
                      ...
                      <div className="contentt" style={{ marginRight: "0px", width: "100vw" }}>
                        ...
                        <div className="redondear" style={{ padding: "6px" }}>
                                ...
                                <div className="menuPCH">
                                  ...
                                  >
                                    <MenuItem
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        navigateToUserMesseges(link.user);
                                        handleClose(link.id);
                                      }}
                                    >
                                      <FontAwesomeIcon icon={faEnvelope} />
                                    ...
                                    {isAdmin && (
                                      <>
                                        <MenuItem
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            setUserVerified(link.user, true);
                                            handleClose(link.id);
                                          }}
                                        >
                                          <FontAwesomeIcon icon={faUserCheck} style={{ marginRight: 8 }} />
                                          Hacer verificado
                                        </MenuItem>
                                        ...
                                        <MenuItem
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            setUserVerified(link.user, false);
                                            handleClose(link.id);
                                          }}
                                        >
                                          <FontAwesomeIcon icon={faUserCheck} style={{ marginRight: 8, opacity: 0.7 }} />
                                          Quitar verificado
                                        </MenuItem>
                                      </>
                                    )}
                                    ...
                                    {isAdmin && (
                                      <>
                                        <MenuItem
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            setUserRecommended(link.user, true);
                                            handleClose(link.id);
                                          }}
                                        >
                                        ...
                                        <MenuItem
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            setUserRecommended(link.user, false);
                                            handleClose(link.id);
                                          }}
                                        >
                                          <FontAwesomeIcon icon={faStar} style={{ marginRight: 8, opacity: 0.6 }} />
                                          Quitar recomendado
                                        </MenuItem>
                                      </>
                                    )}
                                    ...
                                    {isAdmin && (
                                      appHasLiked ? (
                                        <MenuItem onClick={(e) => { e.stopPropagation(); adminAppLikeTask(link.id, false); handleClose(link.id); }}>
                                          <FontAwesomeIcon icon={faStar} style={{ marginRight: 8, opacity: 0.6 }} />
                                          Quitar like App
                                        </MenuItem>
                                      ) : (
                                        <MenuItem onClick={(e) => { e.stopPropagation(); adminAppLikeTask(link.id, true); handleClose(link.id); }}>
                                          <FontAwesomeIcon icon={faStar} style={{ marginRight: 8 }} />
                                          Like App
                                        </MenuItem>
                                      )
                          ...
                          >
                            ...
                            <ul className="users-modal__list">
                              {(listUsersFiltered || []).map((u, i) => {
                                ...
                                return (
                                  <UserRow
                                    key={`likedby-${u.id ?? u.username}-${i}`}
                                    avatarSrc={avatar}
                                    username={u.username}
                                    badgeLabel={badge}
                                    onAvatarError={onAvatarError}
                                    onClick={() => setPeticionajena(u.id)}
                                    right={
                                      <>
                          ...
                          >
                            <TierTabs
                              tabs={tierTabs}
                              counts={shareModalCounts}
                              activeKey={shareModalFilter}
                              onChange={setShareModalFilter}
                            />
