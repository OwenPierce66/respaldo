...
const PerfilesP = ({ mostrarUsuarios, favoritosPerfilesUsuarioSeleccionado, usuarioSeleccionado, mostrarSoloFavoritos, mostrarFormulario, selectedCategory, combinedSearchTerm }) => {
    ...
    const handleLike = async (profileId) => {
        try {
            const token = localStorage.getItem("userTokenLG");
            await axios.post(`http://127.0.0.1:8000/api/profiles/${profileId}/like/`, {}, {
                headers: { Authorization: `Token ${token}` },
            });
...
const navigate = useNavigate();
    ...
    const navigateToUserMesseges = (userId) => {
        navigate.push(`/dashboard/direcmassaging/${userId}`);
    };

    const navigateToUserForum = (userId) => {
        navigate.push(`/dashboard/newcommunity/${userId}`);
    };
...
const cleanVal = (v) => {
  ...
  if (s === "No image available" || s === "undefined" || s === "null" || s === "") return null;
  return s;
};
...
const toSrc = (path) => {
  ...
  // ajusta la base si no usas 127.0.0.1:8000
...
<div className="image-container">
  {cleanVal(usuario.user_image) ? (
    <img
      src={toSrc(usuario.user_image)}
